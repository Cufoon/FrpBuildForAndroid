...
console.log("即将编译 frpc x64_64", versionTag);
...
const CC_PATH = path.join(
  scriptRoot,
  "ndk/toolchains/llvm/prebuilt/windows-x86_64/bin/x86_64-linux-android33-clang.cmd"
);
...
try {
  ...
  const buildTargetPathCMD = JSON.stringify(path.join(buildDir, "frpc-x86_64"));
  ...
  const upxTargetPath = path.join(buildDir, "frpc-x86_64-upx");
